...
void init7Segment(){
    fd7Segment = wiringPiI2CSetup(ADDRESS_7SEGMENT);
    wiringPiI2CWriteReg8(fd7Segment, 0x21, 0x01);
    wiringPiI2CWriteReg8(fd7Segment, 0x81, 0x00);
    wiringPiI2CWriteReg8(fd7Segment, 0xef, 0x00);
    displayClear();
}

void displayDigit(int digit, int value){
    wiringPiI2CWriteReg8(fd7Segment, digit, value);
}
...
void displayColon(int value){
    wiringPiI2CWriteReg8(fd7Segment, DIGIT_COLON, value);
}
